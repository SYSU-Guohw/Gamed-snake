...
from lib.networks.classifier.layers import DropPath, trunc_normal_
...
class WaveMLP(nn.Module):
    def __init__(self, model_name: str = 'T', pretrained: str = None, num_classes: int = 100, *args, **kwargs) -> None:
        super().__init__()
        ...
        for i in range(len(layers)):
            stage = nn.Sequential(*[
                Block(embed_dims[i], mlp_ratios[i], drop_path_rate * (j + sum(layers[:i]) / (sum(layers) - 1)),
                      norm_layer, use_norm)
