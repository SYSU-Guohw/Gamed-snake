...
class Dataset(data.Dataset):
    def __init__(self, ann_file, data_root, split):
        ...
        # random_numbers = random.sample(range(219), 183)
        # random_numbers = random.sample(range(171), 143)
        random_numbers = random.sample(range(230), 184)
        # random_numbers = random.sample(range(1233), 1056) #1232newmask里没有413_image
        # random_numbers = random.sample(range(1,345), 287)
        # population = list(range(1233))
        # population.remove(413)

        # # 生成不包含413的随机样本
        # random_numbers = random.sample(population, 986)
        for i in random_numbers:
            self.train_images_path.append(cfg.train.data_path + '{}_image.jpg'.format(i))
            self.train_masks_path.append(cfg.train.data_path + '{}_mask'.format(i))  # new_data_result 这里没有写完整，完整路径应为：multiple_segmentdata/images600_2/2_mask_1.jpg
#/mnt/date/zhangrch/EnergeSnake/multiple_segmentdata/setA/168_image.jpg
        print('======================')
    ...
    def __getitem__(self, index):
        ...
        if ours:
            ...
            # glob.glob: 函数根据指定的模式来搜索文件路径，并返回一个包含所有匹配路径的列表。（这解释了为什么上面在读入 new_data_result 路径时没有读取完整）
            # 这样做很巧妙，因为我们事先也无法确定图中的掩膜个数有几个
            # 这样看来，img_path 为一个地址字符串，而 mask_paths 为一个地址字符串列表
            ...
            cls_ids=[]
            cla_mask_num = []
            # 遍历 new_data_result 地址列表中的所有元素，将二至 new_data_result 转化为多边形
            #print('1',mask_paths)
            for mask_path in mask_paths:
                #print('2',binary_mask_to_polygon(mask_path))
                polys = binary_mask_to_polygon(mask_path)
                if not polys:  # 检查列表是否为空
                ...
                '''os.path.basename(mask_path): 这个函数调用获取路径字符串 mask_path 的最后一部分，即文件名（不包含目录路径）。
                    split('.')[0]: 该部分将文件名按照.（点）分割成多个部分，并选取第一部分，这通常是为了获取文件名（不包含文件扩展名：jpg或png等）。
                    split('_')[-1]: 接着，将上一步得到的文件名按照'_'（下划线）分割，并选取分割后列表的最后一个元素。这个元素在我们的数据集中为一个数字（1，2，···），隐含表示了对应的脊柱块的ID。
                    int(...): 将上一步得到的字符串转换为整数类型。
                    int(...) - 1: 然后从整数中减去1。这个操作是由于类别ID在我们的数据集中是从1开始的，而编程中通常从0开始计数，因此需要减1来对齐。
                    cls_ids.append(...): 最后，将计算得到的整数（减去1之后的类别ID）添加到列表 cls_ids 的末尾。'''
