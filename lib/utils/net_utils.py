...
def load_model(net, optim, scheduler, recorder, model_dir, resume=True, epoch=-1):
    ...
    # 2. 过滤出共同的keys
    common_keys = {k: v for k, v in pretrained_state.items() if k in model_state}
    
    # 3. 找出缺失的keys并打印警告
    # missing_keys = set(model_state.keys()) - set(pretrained_state.keys())
    # if missing_keys:
    #     print(colored('Warning: Missing keys when loading pretrained model:', 'yellow'))
    #     print('\n'.join(missing_keys))
    #     print('\nThese parameters will be initialized randomly.')
        
    # 4. 使用非严格模式加载共同参数
    #print('Loading compatible parameters...')
    model_state.update(common_keys)
    ...
    #     print(colored(f'Warning: Failed to load optimizer state: {str(e)}', 'yellow'))
        
    try:
        scheduler.load_state_dict(pretrained_model['scheduler'])
    except Exception as e:
        print(colored(f'Warning: Failed to load scheduler state: {str(e)}', 'yellow'))
        
    try:
        recorder.load_state_dict(pretrained_model['recorder'])
    except Exception as e:
        print(colored(f'Warning: Failed to load recorder state: {str(e)}', 'yellow'))
