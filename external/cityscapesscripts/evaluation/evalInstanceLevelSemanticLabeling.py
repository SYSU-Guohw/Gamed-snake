...
# We use this script to evaluate your approach on the vis_GT set.
...
# (one for each vis_GT set image) with the content:
...
# - The label IDs "labelIDPrediction" specify the class of that new_data_result,
...
# confidence score to the new_data_result.
#
    ...
    with open(predInfoFileName, 'r') as f:
        for line in f:
            ...
            if os.path.commonprefix([filename, args.predictionPath]) != args.predictionPath:
                printError(
                    "Predicted new_data_result {} in prediction text file {} points outside of prediction path.".format(filename,
                                                                                                             predInfoFileName))
...
def assignGt2Preds(gtInstancesOrig, gtImage, predInfo, args):
    ...
    # Get a new_data_result of void labels in the groundtruth
    voidLabelIDList = []
    for label in labels:
